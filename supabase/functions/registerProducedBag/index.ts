...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!Number.isFinite(parsedWeight) || parsedWeight <= 0) {
      return json(400, { success: false, error: 'weight_kg must be a positive number' });
    }

    const { data, error } = await supabase.rpc('fn_register_produced_bag', {
      p_tenant_id: tenant_id,
      p_factory_id: factory_id,
      p_production_run_id: production_run_id,
      p_material_id: material_id,
      p_weight_kg: parsedWeight,
      p_location_code: String(location_code || 'FLOOR').trim().toUpperCase(),
      p_created_by: userId,
    });
